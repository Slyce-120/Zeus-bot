...
Versione ottimizzata per Termux (Zero dipendenze native).

Dev by Bonzino (Updated to jimp-compact)
*/
...
/**
 * Ottiene il buffer di un'immagine ottimizzata usando Jimp.
 * @param {string} feature - La chiave dell'immagine richiesta.
...
export async function getThumbBuffer(feature) {
  try {
    ...
    // Controllo se il buffer è già pronto in memoria
    ...
    // Elaborazione (Resize 800x450 + conversione JPEG 80%)
    const buffer = await image
      .cover(800, 450) // 'cover' equivale a { fit: 'cover' } di sharp
      .quality(80)     // Qualità JPEG
      .getBufferAsync("image/jpeg")

    // Salvataggio in cache per usi futuri
